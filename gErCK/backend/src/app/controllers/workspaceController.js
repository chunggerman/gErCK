...
export const getWorkspaceAIConfig = async (req, res) => {
  ...
  try {
    const result = await client.query('SELECT ai_config FROM workspace WHERE id = $1', [id]);
...
export const updateWorkspaceAIConfig = async (req, res) => {
  ...
  const client = await db.connect();
  try {
    await client.query('UPDATE workspace SET ai_config = $1 WHERE id = $2', [JSON.stringify(aiConfig), id]);
